...
namespace SavePoint.BusinessLogic.Services.Interfaces
{
	public interface IUserListService
	{
		...
		Task DeleteListAsync(Guid id, string userId);

		// Game management in lists
		Task AddGameToListAsync(Guid listId, Guid gameId, string userId);
		Task RemoveGameFromListAsync(Guid listId, Guid gameId, string userId);
		Task<bool> IsGameInListAsync(Guid listId, Guid gameId);

		// Voting
		Task VoteOnListAsync(Guid listId, bool isUpvote, string userId);
